...
void save_tee_to_history(const char *terminal_output, const char *ai_analysis) {
    ...
    // 2. Increment sequence ID immediately in main thread to keep order
...
void load_history_to_api(struct json_object *messages_array) {
    ...
    const char *query = "SELECT role, content FROM aiterm_history WHERE is_tee = 0 ORDER BY created_at DESC LIMIT 10";
...
void display_status(AppContext *app) {
    write_to_ai_pane(app, "--- SYSTEM STATUS ---", "\n", "cmd_tag", "body_tag");

    // 1. Check Tee Status
    const char *tee_val = tee_enabled ? "ON" : "OFF";
    const char *tee_tag = tee_enabled ? "green_tag" : "system_tag"; // Green for ON, Red for OFF
    write_to_ai_pane(app, "Tee Logging: ", tee_val, "body_tag", tee_tag);

    // 2. Check Autoreply Status
    const char *auto_val = app->autoreply_enabled ? "ON" : "OFF";
    const char *auto_tag = app->autoreply_enabled ? "green_tag" : "system_tag";
    write_to_ai_pane(app, "Autoreply:   ", auto_val, "body_tag", auto_tag);

    // 3. Check MySQL Connection
    if (app->global_db_conn && mysql_ping(app->global_db_conn) == 0) {
        write_to_ai_pane(app, "Database:    ", "CONNECTED", "body_tag", "green_tag");
    } else {
        write_to_ai_pane(app, "Database:    ", "DISCONNECTED", "body_tag", "system_tag");
    }

    // 4. Check AI Status (Ping a simple endpoint or check last API result)
    // For now, we'll check if the API key/URL is set
    if (app->api_key && strlen(app->api_key) > 0) {
        write_to_ai_pane(app, "AI Status:   ", "READY", "body_tag", "green_tag");
    } else {
        write_to_ai_pane(app, "AI Status:   ", "MISSING CONFIG", "body_tag", "system_tag");
    }
    write_to_ai_pane(app, "---------------------", "\n", "cmd_tag", "body_tag");
}
