...
* **New: Provider abstraction layer** centralizes AI API dispatch, response parsing, and token usage handling.
* **New: OpenAI-compatible providers** can now use the same request engine (OpenAI, Groq, OpenRouter, Mistral, and Ollama).
* **New: Provider configuration defaults** are selected from the `provider=` setting, while `AITERM_PROVIDER_*` environment overrides remain available.
* **Improved: Tee and background AI workers** no longer contain provider-specific dispatch logic.
* **Improved: OpenAI Chat Completions response extraction** now supports `choices[0].message.content`.
* **Fixed: ProviderConfig ownership** now correctly frees the configured provider name.


### Provider switching
The API implementation is selected from the `provider=` setting. OpenAI-compatible providers share the same HTTP/request engine, so changing providers does not require changes to the AI worker, Tee handler, or SNMP feed.

Supported provider names in this alpha:
...
The provider can also be overridden for a test run without editing `aiterm.conf`:

```bash
AITERM_PROVIDER=groq AITERM_MODEL=llama-3.3-70b-versatile ./aiterm
```

Endpoint-level overrides remain available through `AITERM_PROVIDER_BASE_URL`, `AITERM_PROVIDER_ENDPOINT`, `AITERM_PROVIDER_AUTH_HEADER`, `AITERM_PROVIDER_AUTH_SCHEME`, and `AITERM_PROVIDER_QUERY_KEY`.
...
AITerm now includes an **AI Provider Manager** window alongside the existing Session, History, Noise Filter, Policy, and SNMP managers. It provides a graphical interface for selecting and configuring the active AI provider, model, endpoint, authentication settings, and API key.

Built-in provider presets include OpenAI, Gemini, Groq, OpenRouter, Mistral, and Ollama, with a Custom option for compatible endpoints. The manager can test the active configuration and save it to the AITerm configuration file.

The provider layer remains centralized in `ai_provider.c`, so the rest of AITerm does not need provider-specific branches for normal AI requests.
